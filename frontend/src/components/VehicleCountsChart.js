...
const VehicleCountsChart = ({ filters }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(`${process.env.REACT_APP_BACKEND_URL}/api/counts`, {
          params: {
    ...
    <Card variant="outlined">
      <CardContent>
        <Typography variant="h6" component="h2" gutterBottom>
          Vehicle Counts per Hour by Approach
        </Typography>
        <Box height={300}>
...
export default VehicleCountsChart;
